...
//        }
        mModel.getBaner().subscribeOn(Schedulers.io())
                .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .doOnSubscribe(disposable -> {
                ...
                .subscribe(new ErrorHandleSubscriber<String>(mErrorHandler) {
                    ...
                    public void onNext(String s) {
                        Log.d(TAG, "hf_MoviePresenter_onNext1  is  :" + s);
                        mmlist.add(s);
                    }

                    @Override
                    public void onError(Throwable t) {
                        Log.d(TAG, "hf_MoviePresenter_onError1  is  :" + t);
                        super.onError(t);
                    }

                    @Override
                    public void onComplete() {
                        Log.d(TAG, "hf_MoviePresenter_onComplete1  is  :");
                        mRootView.setBanerList(mmlist);
                        super.onComplete();
                    }
                });
